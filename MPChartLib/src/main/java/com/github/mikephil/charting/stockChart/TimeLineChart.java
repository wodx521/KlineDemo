...
public class TimeLineChart extends LineChart {
    ...
    public interface VolSelected {
        void onVolSelected(float value);

        void onValuesSelected(double price, double upDown, float vol, double avg);
    }
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            float yValForXIndex1 = Float.parseFloat(kTimeData.getDatas().get((int) mIndicesToHighlight[i].getX()).getNowPrice());
            float yValForXIndex2 = Float.parseFloat(kTimeData.getDatas().get((int) mIndicesToHighlight[i].getX()).getPer());

            if (volSelected != null) {
                volSelected.onVolSelected(Float.parseFloat(kTimeData.getDatas().get((int) mIndicesToHighlight[i].getX()).getVolume()));
                volSelected.onValuesSelected(Double.parseDouble(kTimeData.getDatas().get((int) mIndicesToHighlight[i].getX()).getNowPrice()),
                        Double.parseDouble(kTimeData.getDatas().get((int) mIndicesToHighlight[i].getX()).getPer()),
                        Float.parseFloat(kTimeData.getDatas().get((int) mIndicesToHighlight[i].getX()).getVolume()),
                        Double.parseDouble(kTimeData.getDatas().get((int) mIndicesToHighlight[i].getX()).getAveragePrice()));
            }
