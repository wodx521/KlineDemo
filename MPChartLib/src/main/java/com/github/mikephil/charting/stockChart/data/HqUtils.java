...
public class HqUtils {
    ...
    public static List<ADX> getAdx(List<KLineDataModel> paramList, int paramInt) {
        ...
        for (int j = 0; j < i; j++) {
            ...
            arrayOfDouble1[j] = localKLineData.getClose();
            arrayOfDouble2[j] = localKLineData.getHigh();
            arrayOfDouble3[j] = localKLineData.getLow();
            arrayOfString[j] = localKLineData.getDateMills();
        }
    ...
    public static ArrayList<KLineDataModel> getmovingAverage
        ...
        for (int j = 0; j < i; j++) {
            ...
            switch (paramInt1) {
                default:
                    break;
                case 4:
                    arrayOfDouble1[j] = localKLineData.getLow();
                    break;
                case 3:
                    arrayOfDouble1[j] = localKLineData.getClose();
                    break;
                case 1:
                    arrayOfDouble1[j] = localKLineData.getHigh();
                    break;
                case 0:
                    arrayOfDouble1[j] = localKLineData.getOpen();
                case 2:
            }
        }
        Core localCore = new Core();
        ...
        if (RetCode.Success == localObject) {
            for (int i1 = 0; i1 < arrayOfDouble4.length; i1++) {
                ...
                ((KLineDataModel) paramArrayList.get(i1)).setMa5((float) arrayOfDouble4[i1]);
            }
        }
        if (RetCode.Success == localRetCode1) {
            for (int n = 0; n < arrayOfDouble4.length; n++) {
                ...
                ((KLineDataModel) paramArrayList.get(n)).setMa10((float) arrayOfDouble4[n]);
            }
        }
        ...
        if (localRetCode3 == localRetCode2) {
            while (m < arrayOfDouble4.length) {
                if (arrayOfDouble4[m] != 0.0D) {
                    ((KLineDataModel) paramArrayList.get(m)).setMa20((float) arrayOfDouble4[m]);
                }
                m++;
            }
        }
