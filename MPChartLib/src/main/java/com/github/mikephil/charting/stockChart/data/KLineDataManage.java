        ...
        if (object != null) {
            ...
            if (data != null) {
                ...
                for (int i = 0; i < data.length(); i++) {
                    ...
                    klineDatamodel.setDateMills("");

                    preClosePrice = klineDatamodel.getPreClose();
                    kDatas.add(klineDatamodel);

                    xVal.add(getKLineDatas().get(i).getDateMills());
                ...
                lineDataMA.add(setALine(ColorType.blue, line5Entries, true));
                lineDataMA.add(setALine(ColorType.yellow, line10Entries, true));
                lineDataMA.add(setALine(ColorType.purple, line20Entries, true));
            }
        }
    }
    ...
    public void parseFeatureKlineData(String result, String assetId, boolean landscape) {
        ...
        if (result != null) {
            ...
            for (int i = 0; i < split.length; i++) {
                ...
                kLineData.setDateMills(split1[6].substring(5, split1[6].length() - 3));
                kLineData.setOpen(Float.parseFloat(split1[0]));
                kLineData.setClose(Float.parseFloat(split1[1]));
                kLineData.setHigh(Float.parseFloat(split1[2]));
                kLineData.setLow(Float.parseFloat(split1[3]));
                kLineData.setVolume(Float.parseFloat(split1[4]));
                maxVolume = (float) Math.max(kLineData.getVolume(), maxVolume);
                // 填充X轴显示数据
//                    xValuesLabel.put(i, kLineData.date);
                // 存储K线数据
                kDatas.add(kLineData);
                // 填充X轴数据
                xVal.add(kLineData.getDateMills());
                // 蜡烛图数据填充
                candleEntries.add(new CandleEntry(i, i + offSet, kLineData.getHigh(), kLineData.getLow(), kLineData.getOpen(), kLineData.getClose()));
                float color = kLineData.getOpen() > kLineData.getClose() ? 0f : 1f;
                barEntries.add(new BarEntry(i, i + offSet, kLineData.getVolume(), color));
            }
            // 设置蜡烛图显示效果
            candleDataSet = setACandle(candleEntries);
            // BOOL图蜡烛图设置
    ...
    public void initBOLL() {
        List<KLineDataModel> maBOLL = HqUtils.getMaBOLL(getKLineDatas(), 20, BOLLN);
        bollDataUP.clear();
        bollDataMB.clear();
        bollDataDN.clear();
    ...
    private LineDataSet setALine(ColorType ma, ArrayList<Entry> lineEntries, String label) {
        return setALine(ma, lineEntries, label, false);
    }
