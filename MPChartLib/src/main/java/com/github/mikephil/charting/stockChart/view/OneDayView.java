...
public class OneDayView extends BaseView {
    ...
    public void setDataToChart(TimeDataManage mData) {
        ...
        ArrayList<TimeDataModel> datas = mData.getDatas();
        setXLabels(mData.getOneDayXLabels(landscape,datas));
        setShowLabels(true);
        setMarkerView(mData);
        setBottomMarkerView(mData);

        axisLeftLine.setAxisMinimum((float) mData.getMin());
        axisLeftLine.setAxisMaximum((float) mData.getMax());
        ...
        leftColorContentYAxisRenderer.setClosePrice(mData.getYesterdayClose());
        ...
        rightColorContentYAxisRenderer.setClosePrice(mData.getYesterdayClose());
        ...
        for (int i = 0, j = 0; i < mData.getDatas().size(); i++, j++) {
            ...
            lineCJEntries.add(new Entry(i, i, Float.parseFloat(mData.getDatas().get(i).getNowPrice())));
            lineJJEntries.add(new Entry(i, i, Float.parseFloat(mData.getDatas().get(i).getAveragePrice())));
            barEntries.add(new BarEntry(i, i, Float.parseFloat(mData.getDatas().get(i).getVolume())));
        }
    ...
    public void dynamicsAddOne(TimeDataModel timeDatamodel, int length) {
        ...
        d1.addEntry(new Entry(index, index, Float.parseFloat(timeDatamodel.getNowPrice())));
        ILineDataSet d2 = lineData.getDataSetByIndex(1);
        d2.addEntry(new Entry(index, index, Float.parseFloat(timeDatamodel.getAveragePrice())));

        BarData barData = barChart.getData();
        IBarDataSet barDataSet = barData.getDataSetByIndex(0);
        barDataSet.addEntry(new BarEntry(index, index, Float.parseFloat(timeDatamodel.getVolume())));
