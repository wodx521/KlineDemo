...
public class MyCombinedChart extends CombinedChart {
    ...
    public void setMarker(BarBottomMarkerView markerBottom, FeatureKLineDataManage kLineData, TimeType timeType) {
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            String date = kLineData.getKLineDatas().get((int) e.getX()).date;

//            if (this.timeType == TimeType.TIME_HOUR) {
//                date = DataTimeUtil.secToTime(kLineData.getKLineDatas().get((int) e.getX()).date);
//            } else {
//                date = DataTimeUtil.secToDate(kLineData.getKLineDatas().get((int) e.getX()).date);
//            }
