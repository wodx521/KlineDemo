        ...
        if (kLineBeen != null && kLineBeen.size() > 0) {
            {
                for (int i = 0; i < kLineBeen.size(); i++) {
                    int index = n - 1;
                    typ = (float) ((kLineBeen.get(i).high + kLineBeen.get(i).low + kLineBeen.get(i).close) / 3);
                    if (i >= index) {
                        ma = getSum(i - index, i, kLineBeen) / n;
                        avedev = getSMA(avedev, (float) Math.abs(kLineBeen.get(i).close - ma), n);
                    } else {
                        ma = defult;
                        avedev = (float) Math.abs(kLineBeen.get(i).close);
                    }
