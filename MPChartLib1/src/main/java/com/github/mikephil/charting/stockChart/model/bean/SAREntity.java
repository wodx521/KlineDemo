...
import com.github.mikephil.charting.stockChart.data.HqUtils;
        ...
        for (int index = 0; index < datas.size(); index++) {
            ...
            if (index - max_index == num) {
                ...
                for (int j = index - num + 1; j < index; j++) {
                    ...
                    float c = (float) datas.get(j).close;
                    int v = (int) datas.get(j).volume;
...
//                        datas.get(j).extVal["\(self.key(CHSeriesKey.volume))"] = max_vol;
                ...
                //每位移一个数，计算是否最大交易量
                float c = (float) datas.get(index).close;
                int v = (int) datas.get(index).volume;
                if (v > max_vol) {
                    max_vol_price = c;
                    max_vol = v;
                    max_index = index;
