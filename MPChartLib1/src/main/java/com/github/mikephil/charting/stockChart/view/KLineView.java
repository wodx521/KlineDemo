...
import com.github.mikephil.charting.stockChart.CustomMarkerView;
...
public class KLineView extends BaseView {
    ...
    private FeatureKLineDataManage kLineData;
    ...
    public void initChart(boolean landscape) {
        ...
        axisLeftK.setTextColor(ContextCompat.getColor(mContext, R.color.chart_bg));
        ...
        axisLeftBar.setTextColor(ContextCompat.getColor(mContext, R.color.chart_bg));
     ...
     * 设置K线数据
     */
    public void setDataToChart(FeatureKLineDataManage data) {
        kLineData = data;
...
//                break;
    ...
    private void setADXToChart() {
        if (barChart != null) {
            ...
            CombinedData combinedData = barChart.getData();
            combinedData.setData(new LineData(kLineData.getLineDataADX()));
            barChart.notifyDataSetChanged();
            barChart.invalidate();
        }
    }
    ...
    private void setATRToChart() {
        if (barChart != null) {
            ...
            CombinedData combinedData = barChart.getData();
            combinedData.setData(new LineData(kLineData.getLineDataATR()));
            barChart.notifyDataSetChanged();
            barChart.invalidate();
        }
    }

    public void setMarkerView(FeatureKLineDataManage kLineData) {
        CustomMarkerView customMarkerView = new CustomMarkerView(mContext, R.layout.custom_markerview, precision);
        candleChart.setMarker(customMarkerView, kLineData);
    }

    public void setBottomMarkerView(FeatureKLineDataManage kLineData) {
    ...
    public void updateText(int index, boolean isSelect) {
        ...
        candleChart.setDescriptionCustom(zbColor, new String[]{"MA5:" + NumberUtils.keepPrecision(kLineData.getKmaEntity5().getMAs().get(index), 3), "MA10:" + NumberUtils.keepPrecision(kLineData.getKmaEntity10().getMAs().get(index), 3), "MA20:" + NumberUtils.keepPrecision(kLineData.getKmaEntity20().getMAs().get(index), 3)});
        chartSwitch(index);
    }
    ...
    private void chartSwitch(int index) {
        switch (chartType1) {
            case 1:
                barChart.setDescriptionCustom(ContextCompat.getColor(mContext, R.color.label_text), getResources().getString(R.string.vol_name) + NumberUtils.formatVol(mContext, kLineData.getAssetId(), kLineData.getKLineDatas().get(index).volume));
                break;
            case 2:
                barChart.setDescriptionCustom(ContextCompat.getColor(mContext, R.color.ma20), "MACD:" + (kLineData.getMacdData().size() <= index ? "--" : NumberUtils.keepPrecision(kLineData.getMacdData().get(index).getY(), 3)));
                break;
            case 3:
...
//                break;
            case 4:
                ...
                break;
            case 5:
                barChart.setDescriptionCustom(ContextCompat.getColor(mContext, R.color.label_text), "ADX:" + (kLineData.getAdxData().size() <= index ? "--" : NumberUtils.keepPrecision(kLineData.getAdxData().get(index).getY(), 2)));
                break;
            case 6:
                barChart.setDescriptionCustom(ContextCompat.getColor(mContext, R.color.label_text), "ATR:" + (kLineData.getAtrData().size() <= index ? "--" : NumberUtils.keepPrecision(kLineData.getAtrData().get(index).getY(), 2)));
                break;
            default:
                barChart.setDescriptionCustom(ContextCompat.getColor(mContext, R.color.label_text), getResources().getString(R.string.vol_name) + NumberUtils.formatVol(mContext, kLineData.getAssetId(), kLineData.getKLineDatas().get(index).volume));
                break;
